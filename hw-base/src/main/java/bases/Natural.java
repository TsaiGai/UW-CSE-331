...
public class Natural {
    ...
    public static int leadingDigit(int[] digits) {
        ...
        // Inv: 0 < D[i] <= n, 0 < D[i-1] <= n, ..., 0 < D[0] <= n, and
        // 0 != D[i], 0 != D[i-1], ... 0 != D[0]
        // TODO: write your loop here
        while (i != 0) {
            i = i - 1;
            if (digits[i] != 0) {
                // At this point of the code, we know that a non-zero value in D has been reached
                return i;
            }
        }

        // At this point in the code, we know that all the values in D are zero
        // and D has been iterated through.
        // This implies the postcondition below, since a non-zero value in D hasn't been reached and i = 0.
    ...
    public Natural toBase(int base) {
        ...
        //      where D = this.digits, n = this.digits.length, and b = this.base.
        while (i != 0) {  // TODO: Replace the condition here with a suitable one.

            // TODO: Implement the body of this loop, so that it's correct with the given invariant.
            i = i - 1;
            j = j + 1;
            r = r.plus(b).times(new Natural(base, this.digits[i]));
        }
        r = new Natural(base, getValue());

        // TODO: Explain why the postcondition holds at the end of this code.
        // Post: r = (base, this.value)
        // Explanation - since the code has exited the while loop, we know that i = 0.
        // Therefore, j = n - 1, and based on the getValue method, j = n - 1 returns this.value.
        return r;
    }
    ...
    public String toString() {
       ...
       // TODO: write your loop here
        while (i != 0) {
            i = i - 1;
            char c = BaseDigits.digitToChar(digits[i], 10);
            buf.append(c);
        }

        // At this point in the code, we know that buf contains the digits in D in a string format.
        // This implies the postcondition below, since buf equals the digits (as chars) from each index of D.
    ...
    public Natural plus(Natural other) {
        ...
        // Summary - the loop is designed to construct a new digits array that stores
        // the sum of this.digits and other.digits
        ...
        // Explanation - since 0 <= A[i] < b and 0 <= B[i] < b, then 0 <= A[i] + B[i] < 2b which
        // is equivalent to 0 <= D[i] < 2b, and therefore 0 <= D[i-1] < 2b-1.

        // TODO: Explain why the invariant of the loop below holds initially (no code needed).
        // Explanation - since we have exited out of the first while loop, the length of other.digits
        // has been reached. However, we have yet to iterate through the rest of this.digits.

        // TODO: Summary comment here
        // Summary - the loop is designed to add the rest of this.digits to the new digits array
        // Inv: D[0] = A[0]+B[0], D[1] = A[1]+B[1], ..., D[i-1] = A[i-1]+B[i-1],
        ...
        // Explanation - this.value = A[0] + A[1] b + ... + A[n-1] b^{n-1} and
        // other.value = B[0] + B[1] b + ... + B[n-1] b^{n-1}, so since
        // A[n-1] + B[n-1] = D[n-1], the above statement holds.
        ...
        // Explanation 1 - adding values from this.digits and other.digits are demonstrated
        // through the first two loops, and making sure each digit in the new digits array
        // is between 0 and b-1 is demonstrated through the last loop.
        // Explanation 2 - the base was never changed while passing through the loops, the
        // length of the new digits array is greater than 1, and 0 < D[n-1] < 2b-1.
